...
export class ErrorStatus implements BaseCode {
    ...
    static readonly MEMBER_INFO_NOT_FOUND = new ErrorStatus(false, 400, "사용자 입력 정보가 부족합니다.");
    static readonly EMAIL_ALREADY_TAKEN = new ErrorStatus(false, 400, '이 이메일은 이미 사용 중입니다.');
    ...
    static readonly FAVORITE_REQUEST_NOT_FOUND = new ErrorStatus(false, 400, '관심 사용자 등록 요청이 존재하지 않습니다.');
    ...
    static readonly EVENT_NOT_FOUND = new ErrorStatus(false, 400, "해당 이벤트가 존재하지 않습니다.");
     ...
     //S3 에러
     static readonly S3_UPLOAD_FAILURE = new ErrorStatus(false, 400, "이미지 업로드에 실패했습니다.");
}